...
As a user, you can also create a workspace containing a clone of a Git repository by entering the URL of the Git repository on the {prod-short} dashboard page.
...
Clicks on a {prod-short} URL link create a fresh workspace when no previous matching workspace exists.

Use `?new` at the end of the URL to always create a fresh workspace on every click

Example: `https://che-server#https://github.com/example/foo?new` to create a new workspace from this repository.
